...
from q2_matchmaker._stan import _case_control_data, _case_control_lognormal_data
from q2_matchmaker._stan import _case_control_full, _case_control_lognormal
...
def lognormal_case_control(
    ...
    metadata = metadata.loc[idx]
    depth = counts.sum(axis=1)
    dat = _case_control_lognormal_data(counts.values,
                             metadata['cc_ids'].values,
                             metadata['groups'].values, depth)
    _, posterior = _case_control_lognormal(
        counts=counts.values,
        case_ctrl_ids=metadata['cc_ids'].values,
        case_member=metadata['groups'].values,
        mc_samples=monte_carlo_samples)
